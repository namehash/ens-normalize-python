...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            f"a{chr(173)}",
            NormalizableSequenceType.IGNORED,
            1,
            chr(173),
            "",
...
def test_normalization_error_object():
    ...
    try:
        ...
        assert (
            repr(e)
            == 'CurableSequence(code="UNDERSCORE", index=1, sequence="_", suggested="")'
        )
    try:
        ens_normalize("0х0")
    except DisallowedSequence as e:
...
def test_error_meta():
    ...
    assert (
        e.general_info
        == "Contains visually confusing characters from multiple scripts (Greek/Latin)"
    )
    assert (
        e.sequence_info
    ...
    assert (
        e.general_info
        == "Contains visually confusing characters from Cyrillic and Latin scripts"
    )
    ...
    assert (
        e.general_info
        == "Contains visually confusing characters from multiple scripts (Latin plus other scripts)"
    )
    assert (
        e.sequence_info
...
def test_ens_cure_max_iters(mocker):
    mocker.patch(
        "ens_normalize.normalization.ens_normalize", lambda _: ens_normalize("?")
    )
