...
def create_emoji_regex_pattern(emojis: List[str]) -> str:
    ...
    return "|".join(
        make_emoji(emoji) for emoji in sorted(emojis, key=order, reverse=True)
    )
...
class NormalizationData:
    def __init__(self, spec_json_path: str):
        ...
        self.mapped: Dict[int, List[int]] = {
            cp_src: mapping for cp_src, mapping in spec["mapped"]
        }
        ...
        self.emoji_fe0f_lookup = create_emoji_fe0f_lookup(
            ["".join(chr(cp) for cp in cps) for cps in self.emoji]
        )
        self.emoji_regex = re.compile(
            create_emoji_regex_pattern(
                ["".join(chr(cp) for cp in cps) for cps in self.emoji]
            )
        )
        ...
        if token.type in (TY_VALID, TY_MAPPED):
            if cps_requires_check(token.cps):
                ...
                cps = [
                    cp
                    for tok in slice
                    if tok.type in (TY_VALID, TY_MAPPED)
                    for cp in tok.cps
                ]
                str0 = cps2str(cps)
                str = NFC(str0)
...
def post_check_hyphen(label: str) -> Optional[CurableSequence]:
    if (
        len(label) >= 4
        and all(ord(cp) < 0x80 for cp in label)
        and "-" == label[2] == label[3]
    ):
        return CurableSequence(
...
def post_check_group_whole(
    cps: List[int], is_greek: List[bool]
) -> Optional[Union[DisallowedSequence, CurableSequence]]:
    ...
    return post_check_group(g, cps_no_fe0f, cps) or post_check_whole(g, unique)
...
def determine_group(
    unique: Iterable[int], cps: List[int]
) -> Tuple[Optional[List[Dict]], Optional[CurableSequence]]:
...
def post_check_group(
    g, cps: List[int], input: List[int]
) -> Optional[Union[DisallowedSequence, CurableSequence]]:
    v, m = g["V"], g["M"]
    for cp in cps:
        if cp not in v:
                    ...
                    for k in range(i, j):
                        if decomposed[k] == decomposed[j]:
                            return DisallowedSequence(
                                DisallowedSequenceType.NSM_REPEATED
                            )
                    j += 1
                i = j
            i += 1
...
def post_check(
    name: str, label_is_greek: List[bool], input: str
) -> Optional[Union[DisallowedSequence, CurableSequence]]:
    ...
    for label in name.split("."):
        ...
        if e is not None:
            ...
            if isinstance(
                e, CurableSequence
            ):  # or NormalizableSequence because of inheritance
...
def find_normalizations(tokens: List[Token]) -> List[NormalizableSequence]:
    ...
    for tok in tokens:
        ...
        if warning is not None:
            warnings.append(
                NormalizableSequence(warning, start, disallowed, suggestion)
            )
            warning = None
        start += scanned
    return warnings
...
def tokens2str(
    tokens: List[Token],
    emoji_fn: Callable[[TokenEmoji], str] = lambda tok: cps2str(tok.cps),
) -> str:
    t = []
    for tok in tokens:
            ...
            if tok.type in (TY_IGNORED, TY_DISALLOWED):
                ...
                if not label_is_greek[label_index]:
                    s.append(
                        cps2str(
                            [
                                CP_XI_CAPITAL if cp == CP_XI_SMALL else cp
                                for cp in tok.cps
                            ]
                        )
                    )
